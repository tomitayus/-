...
def parse_wish_sheet(xls):
    """「希望」シート → (marks {正規化医師名:{date:段階}}, invalid [(date,医師,値)])。

    シート無し/空なら ({}, [])。数値・空欄は無視、不正な文字列マークだけ invalid。
    """
    wish_sheet = find_sheet_name(xls, "希望")
    if wish_sheet is None:
        return {}, []
    raw = strip_cols(pd.read_excel(xls, sheet_name=wish_sheet))
    raw.columns = make_unique(list(raw.columns))
    if raw.shape[1] < 2 or raw.shape[0] == 0:
        return {}, []
    dcol = raw.columns[0]
    raw[dcol] = pd.to_datetime(raw[dcol], errors="coerce").dt.normalize().dt.tz_localize(None)
    marks = {}
    invalid = []
    for col in raw.columns[1:]:
        doc = normalize_name(col)
        dd = {}
        for _, row in raw.iterrows():
            dt = row[dcol]
            v = row[col]
            if pd.isna(dt) or pd.isna(v) or not isinstance(v, str):
                continue
            if not v.strip():
                continue
            st = parse_wish_mark(v)
            if st:
                dd[dt] = st
            else:
                invalid.append((dt, col, v))
        if dd:
            marks[doc] = dd
...
def preflight_validate(shift_df, date_col_shift, availability_df, doctor_names,
                       ...
                       wish_marks=None, invalid_wish=None):
    ...
    if wish_marks:
        ...
        if _unknown:
            warnings.append(f"「希望」シートの医師が名簿(sheet2)に無い（その希望は無視）: {_unknown}"
                            " ※表記ゆれは config.NAME_ALIASES で対応可")
        _conflicts = []
        for _d, _dm in wish_marks.items():
            for _dt in _dm:
                if not _avail_ok(_dt, _d):
                    _conflicts.append((_dt, _d))
        if _conflicts:
...
def run(input_path, output_dir=None, num_patterns=None):
    """当直表生成パイプラインを実行する（読込→生成→最適化→Excel出力）。

    Args:
        input_path: 入力Excelファイル（Tochoku雛形）のパス
        output_dir: 出力先ディレクトリ（省略時は ~/Downloads）
        num_patterns: 生成パターン数（省略時は config.NUM_PATTERNS）

    Returns:
        str: 出力xlsxファイルのパス

    Note:
        従来のモジュールレベル実行と互換にするため、パイプライン内で束縛される
        名前はすべて global 宣言でモジュール名前空間に公開している
        （'x' in globals() による判定や関数間のグローバル参照を維持するため）。
    """
    global NUM_PATTERNS, uploaded_filename, xls, e, sheet1_name, sheet2_name, sheet3_name, sheet4_name, missing, shift_df
    global availability_raw, schedule_raw, sheet4_raw_out, sheet4_grid, sheet4_data, date_col_shift, date_col_avail, availability_df, date_col_sched, schedule_df
    global _BUILTIN_HOLIDAYS, _sched_dates_for_holiday, _holiday_source, _target_ym, jpholiday, _y, _m, _hd, _hname, _hs
    global _ht, _h, HOLIDAYS, is_holiday, _period_holidays, doctor_names, doctor_col_index, WED_FORBIDDEN_DOCTORS, KATE_TOBAN_COL, all_cols
    global col, hospital_cols, n_cols, B_COL_INDEX, C_COL_INDEX, D_COL_INDEX, E_COL_INDEX, F_COL_INDEX, G_COL_INDEX, H_COL_INDEX
    global I_COL_INDEX, J_COL_INDEX, K_COL_INDEX, L_COL_INDEX, M_COL_INDEX, Q_COL_INDEX, U_COL_INDEX, Y_COL_INDEX, B_H_START_INDEX, B_H_END_INDEX
    global I_K_START_INDEX, I_K_END_INDEX, B_K_START_INDEX, B_K_END_INDEX, L_Y_START_INDEX, L_Y_END_INDEX, _invalid_avail_marks, _missing_avail_cols, doc, _dt
    global _v, _f, _ok, _doc, get_avail_code, get_sched_code, sched_doctors, only2, only3, name_to_row
    global prev_names, match_prev_name, name_match, unmatched, prev_get, prev_total, prev_bg, prev_ht, prev_weekday, prev_weekend
    global prev_get_str, expected_team_codes, ridx, team_val, team_str, kate_team_col_name, col_candidate, col_values, idx, row
    global v, val_str, doctor_kate_team, doctor_attribute, attr_val, attr0, attr1, attr2, attr3, attr_none
    global parts, travel_col_name, doctor_travel_day, WEEKDAY_MAP, get_travel_weekday, get_pre_travel_dates, doc_with_attr, doc_with_travel, kate_team_by_date, date
    global unique_teams, slots_by_date, preassigned_count, total_slots, hosp, val, all_dates, all_shift_dates, is_always_unavailable, inactive_doctors
    global active_doctors, has_code_2_anywhere, CODE_2_DOCTORS, BASE_TARGET, EXTRA_SLOTS, active_sorted_by_index, attr1_doctors, EXTRA_ALLOWED, remaining, non_attr1
    global TARGET_CAP, d, compute_max_gap3_assignments, gap3_cap_adjusted, max_gap3, total_cap, shortage, _redist_candidates, floor_shifts, total_cap_final
    global extra_names, extra_attr1_count, gap3_limited, has_sheet3_code_3, has_any_schedule_code, RATIO_EXEMPT_DOCTORS, SCHEDULE_CODE_HOLDERS, NO_KATE_DOCTORS, sample_teams, has_sheet3_code_1
    global SHEET3_CODE_1_DOCTORS, is_ch_slot, is_weekday_university_slot, is_eligible_for_ch_slot, is_eligible_for_weekday_university_slot, is_cc_assignment, has_any_cc_assignment, has_code_1_2, CODE_1_2_DOCTORS, UNIVERSITY_MINIMUM_REQUIRED_DOCTORS
    global is_bg_day_shift, is_bk_slot, is_ly_slot, classify_bg_category, choose_doctor_for_slot, build_schedule_pattern, slot_meta, movable_positions, is_preassigned_slot, get_bg_week_start
    global get_monday_week_start, recompute_stats, evaluate_schedule_with_raw, can_assign_doc_to_slot, is_valid_full_assignment, build_date_doc_count, collect_violation_docs_from_assignments, is_better_raw, local_search_swap, META_COLS_SHEET4
    global BASE_SUMMARY_COLS, UNIV7_SET, UNIV7_ORDER, DETAIL_COLS, SUMMARY_DETAIL_COLS, SUMMARY_COLS, count_doc_in_column, build_summaries, build_gap_details, build_same_day_duplicates
    global build_hosp_dup_details, build_weekly_bg_details, build_unassigned_details, build_doctor_diag, build_metrics_df, build_hard_constraint_violations, fix_hard_constraint_violations, fix_target_cap_violations, fix_code_2_extra_violations, fix_university_minimum_requirement
    global fix_ch_kate_violations, fix_bg_ht_imbalance_violations, fix_gap_violations, fix_external_hospital_dup_violations, fix_university_over_2_violations, fix_weekly_bg_violations, fix_university_weekday_balance_violations, fix_weekday_weekend_balance, fix_fairness_imbalance, fix_unassigned_slots
    global validate_absolute_constraints, safe_fix, build_diagnostics, score_rows, candidates, i, pattern_df, counts, bg_counts, ht_counts
    global wd_counts, we_counts, bk_counts, ly_counts, bg_cat, score, raw_score, metrics, gap_violations, cand
    global _, refined, refine_list, improved_df, sc2, raw2, met2, current_df, total_fix_counts, MAX_ROUNDS
    global opt_round, round_fixed, result, fc, fail_count, final_df, final_gap_fc, final_dup_fc, final_weekly_bg_fc, final_cap_fc
    global final_wd_we_fc, final_unassigned_fc, final_weekly_bg_fc2, fix_count, code_2_fix_count, cap_fix_count, univ_min_fix_count, ch_kate_fix_count, gap_fix_count, bg_ht_fix_count
    global ext_dup_fix_count, univ_over_2_fix_count, univ_weekday_fix_count, fairness_fix_count, unassigned_fix_count, any_fixed, unassigned_success, counts_tmp, bg_tmp, ht_tmp
    global wd_tmp, we_tmp, bk_tmp, ly_tmp, bg_cat_tmp, violations, is_valid, abs_valid_count, abs_invalid_count, valid_patterns
    global excluded_count, met, cap_viol, gap_viol, unassigned, code_2_viol, bg_over_2_viol, ht_0_viol, wd_we_viol, we_0_viol
    global abs_valid, fairness_patterns, gap_patterns, balance_patterns, abs_valid_patterns, fairness_abs, gap_abs, balance_abs, axis_labels, axis_bests
    global top_patterns, seen_seeds, label, best, p, refined_sorted, TOP_OUTPUT_PATTERNS, scores_df, refined_df, pattern
    global fairness, abs_viols, seed, abs_str, base_name, output_filename, _downloads_dir, output_path, _fmt_date_jp, _fmt_date_cols
    ...
    preflight_validate(
        ...
        wish_marks=_wish_marks,
        invalid_wish=_invalid_wish,
    )
    ...
    if output_dir is not None:
        _downloads_dir = os.path.expanduser(str(output_dir))
    else:
...
def _resolve_input_path_cli():
    """入力ファイルパスの決定（コマンドライン引数 > Finderダイアログ > config.py）"""
    if len(sys.argv) > 1:
        return sys.argv[1]
    try:
